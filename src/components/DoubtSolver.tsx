...
export function DoubtSolver() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Hello! I am your advanced AI tutor for KSP Master. Ask me any doubt from Mathematics, General Science, Reasoning, or General Awareness. You can also upload a screenshot of a question!' }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let contents: any = userText || 'Explain this image in the context of Karnataka Police Constable (KSP) exam.';
      
      if (currentImage) {
        contents = {
          parts: [
            { text: userText || 'Explain this image in the context of Karnataka Police Constable (KSP) exam.' },
            {
              inlineData: {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: contents,
        config: {
          systemInstruction: `You are a Karnataka Police Constable (KSP) exam expert solver for Mathematics and Reasoning questions. Follow this EXACT output format for every Math or Reasoning question:
