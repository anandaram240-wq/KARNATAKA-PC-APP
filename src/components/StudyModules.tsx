...
function TopicCard({
  ...
  return (
    <div className={cn(
      'rounded-2xl border overflow-hidden transition-all duration-300 bg-surface-container',
      open ? 'shadow-xl border-white/10' : 'shadow-sm border-white/5 hover:border-white/10 hover:shadow-md'
    )}>
      {/* ── Card header (always visible) ── */}
      <button
        className="w-full text-left p-4 bg-white/5 hover:bg-white/10 transition-colors"
        onClick={() => setOpen(o => !o)}
      >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <span className={cn('text-[9px] font-black px-2 py-0.5 rounded-full border uppercase tracking-wider', 
                count >= 80 ? 'bg-red-500/10 text-red-400 border-red-500/20' :
                count >= 40 ? 'bg-amber-500/10 text-accent border-amber-500/20' :
                count >= 20 ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                              'bg-white/5 text-on-surface-variant border-white/5'
              )}>
                {count >= 80 ? 'MUST DO 🔥' : count >= 40 ? 'HIGH VALUE ⚡' : count >= 20 ? 'IMPORTANT' : 'GOOD TO KNOW'}
              </span>
              <span className={cn('text-[9px] font-black px-2 py-0.5 rounded-full border border-white/5 bg-white/5 text-on-surface-variant')}>
                {count} PYQs
              </span>
              <span className="text-[9px] text-on-surface-variant font-bold">{pct}% of subject</span>
              {hasDiagram && (
                <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-primary/20 text-indigo-300 border border-primary/30">
                  📊 Visual
                </span>
              )}
            </div>
            <h3 className="font-bold text-on-surface text-sm sm:text-base leading-tight">{topic}</h3>
            {master && (
              <p className="text-xs text-on-surface-variant mt-1.5 line-clamp-1">
                <span className="font-bold text-secondary">⚡</span> {master.trick}
              </p>
            )}
          </div>
      ...
      {open && (
        <div className="border-t border-white/5">
            ...
            {hasDiagram && (
              <div className="sm:w-56 shrink-0">
                <button
                  ...
                  className="w-full text-[10px] font-black text-indigo-300 bg-primary/10 border border-primary/20 rounded-xl px-3 py-1.5 mb-2 hover:bg-primary/20 transition-colors"
                >
                  {showDiagram ? '🔼 Hide Diagram' : '📊 Show Visual Diagram'}
                </button>
                {showDiagram && (
                  <div className="rounded-xl border border-white/5 overflow-hidden bg-white/5 aspect-[220/80]">
              ...
              <div className={cn('flex-1', !hasDiagram && 'sm:w-full')}>
                <button
                  ...
                  className={cn(
                    ...
                    'bg-gradient-to-r font-black text-white text-sm shadow-lg',
                    'hover:opacity-90 hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 border border-white/10',
                ...
                <p className="text-[10px] text-on-surface-variant text-center mt-1.5">
                  Opens {count} real KSP PYQs filtered for this topic
                </p>
              </div>
            )}
          </div>
          ...
          {/* MASTERCLASS box */}
          {master && (
            <div className="p-4 bg-white/5 border-b border-white/5 mt-3">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {/* Speed trick */}
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3">
                  <p className="text-[9px] font-black text-accent uppercase tracking-widest mb-1.5">⚡ Speed Trick</p>
                  <p className="text-xs text-amber-200/90 font-semibold leading-relaxed">{master.trick}</p>
                </div>
                {/* Formula */}
                <div className="bg-primary-container/35 border border-primary/20 rounded-xl p-3">
                  <p className="text-[9px] font-black text-indigo-300 uppercase tracking-widest mb-1.5">📐 Key Formula</p>
                  <p className="text-xs font-mono text-indigo-200/90 font-bold leading-relaxed">{master.formula}</p>
                </div>
                {/* Exam note */}
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3">
                  <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-1.5">🎯 KSP Exam Note</p>
                  <p className="text-xs text-emerald-200/90 font-semibold leading-relaxed">{master.examNote}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {sampleQ ? (
            <div className="p-4">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-6 h-6 rounded-lg bg-white/5 flex items-center justify-center border border-white/15">
                  <BookOpen size={12} className="text-secondary" />
                </div>
                <p className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest">
                  Sample PYQ — {sampleQ.exam_year} {sampleQ.shift}
                </p>
                <span className={cn('text-[9px] font-black px-2 py-0.5 rounded-full ml-auto uppercase border',
                  sampleQ.difficulty === 'easy'   ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                  sampleQ.difficulty === 'hard'   ? 'bg-red-500/10 text-red-400 border-red-500/20'         :
                ...
                {sampleQ.options.map((opt, idx) => (
                  <div
                    ...
                    className={cn(
                      ...
                      idx === sampleQ.correctAnswer
                        ? 'bg-green-500/10 border-green-500/35 font-bold text-green-300'
                  ...
                  >
                    <span className={cn(
                      ...
                      idx === sampleQ.correctAnswer ? 'bg-green-500 text-white border-green-500' : 'bg-white/5 text-on-surface-variant border-white/5'
                    )}>
                      {String.fromCharCode(65 + idx)}
                    </span>
                    <span className="leading-tight">{cleanText(opt)}</span>
                    {idx === sampleQ.correctAnswer && (
                      <span className="ml-auto text-[9px] bg-green-600 text-white px-1.5 py-0.5 rounded font-black shrink-0">✓ ANS</span>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="p-4 text-center text-sm text-on-surface-variant/50 italic">
...
export function StudyModules({ onPractice }: StudyModulesProps) {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto text-on-surface">

      {/* ── PAGE HEADER ───────────────────────────────────────────────── */}
      <div className="relative overflow-hidden rounded-3xl bg-surface-container border border-white/5 p-6 sm:p-8 shadow-xl">
        <div className="absolute -top-10 -right-10 w-48 h-48 bg-primary/10 rounded-full filter blur-xl" />
        <div className="relative flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[9px] font-black text-secondary uppercase tracking-widest bg-secondary/10 px-2.5 py-0.5 rounded-full border border-secondary/20">
                KSP PC Exam Specialist
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-on-surface tracking-tight">ವಿಷಯವಾರು ಅಭ್ಯಾಸ (Subject Mastery)</h1>
            <p className="text-xs sm:text-sm text-on-surface-variant mt-1.5">
              Only question types that actually appear in KSP exams — detailed tricks, formulas, diagrams & original PYQs.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-5">
          {SUBJECTS.map(s => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-2.5 bg-white/5 px-3 py-1.5 rounded-xl border border-white/5">
                <span className="text-base">{s.icon}</span>
                <div>
                  <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider leading-none">{s.id}</p>
                  <p className="text-xs font-black text-on-surface mt-0.5 leading-tight">{cnt} PYQs</p>
                </div>
              </div>
            );
          })}
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive
                  ? 'border-transparent shadow-lg scale-[1.02] bg-gradient-to-br from-primary to-indigo-700'
                  : 'border-white/5 hover:border-white/10 hover:shadow-md bg-surface-container'
              )}
            >
              <div className="relative z-10">
                <span className="text-2xl mb-2 block">{s.icon}</span>
                <p className={cn('font-black text-sm leading-tight', isActive ? 'text-white' : 'text-on-surface')}>
                  {s.label}
                </p>
                <p className={cn('text-[10px] font-bold mt-1', isActive ? 'text-white/70' : 'text-on-surface-variant')}>
      ...
      <div className="rounded-2xl p-4 border border-white/5 bg-white/5 flex flex-wrap items-center gap-4">
        <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center text-xl bg-gradient-to-br shrink-0 shadow-md', subjectCfg.color)}>
          {subjectCfg.icon}
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-black text-on-surface text-sm">{activeSubject}</p>
          <p className="text-[11px] text-on-surface-variant mt-0.5">{topicData.length} Topics · {totalSubjectQs} PYQs in database</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          {mustDoCount > 0 && (
            <div className="flex items-center gap-1.5 bg-red-500/10 border border-red-500/20 px-3 py-1.5 rounded-xl">
              <Star size={12} className="text-red-400" fill="currentColor" />
              <span className="text-xs font-bold text-red-400">{mustDoCount} Must-Do Topics</span>
            </div>
          )}
          {highValCount > 0 && (
            <div className="flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-xl">
              <Zap size={12} className="text-accent" />
              <span className="text-xs font-bold text-accent">{highValCount} High-Value Topics</span>
            </div>
          )}
        </div>
      </div>
      ...
      {/* ── EXAM STRATEGY ALERT ───────────────────────────────────────── */}
      <div className="bg-surface-container border border-white/5 rounded-3xl p-5 flex items-start gap-4 shadow-md">
        <Target size={20} className="text-secondary shrink-0 mt-0.5" />
        <div>
          <p className="text-on-surface font-black text-sm mb-1">
            🎯 KSP PC Exam Strategy — {activeSubject}
          </p>
          <p className="text-on-surface-variant text-xs leading-relaxed mt-1">
            {activeSubject === 'Mathematics' && 'Mathematics covers key logic and equations. Focus on speed conversion, LCM/ratio concepts, average calculation shortcuts, and mensuration formulas first. Doing so guarantees high performance in numerical drills.'}
            {activeSubject === 'Reasoning' && 'Mental Ability & Reasoning yields 15 vital marks! Number series, direction mapping, and coding puzzles are recurring favorites. Practice visualization techniques to solve questions under 30 seconds.'}
            {activeSubject === 'General Science' && 'General Science (Physics, Chemistry, Biology) makes up 15 marks. Class 9–10 NCERT level topics like Lens refraction, Ohm\'s law, Periodic table properties, and human organ functions are highly tested.'}
            {activeSubject === 'General Awareness' && 'General Awareness holds the highest weightage (approx 50+ marks in KSP exams). Memorize Indian Constitution articles, Karnataka history Unification (Ekikarana), local rivers, and current state affairs.'}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white/5 border border-white/5 rounded-3xl p-5">
        <p className="text-secondary font-black text-sm mb-3">📚 How to use these modules:</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { step: '1', icon: '🔥', text: 'Open MUST DO topics first — they appear in almost every KSP exam shift' },
            { step: '2', icon: '📊', text: 'Toggle the Visual Diagram to master complex concepts in 10 seconds' },
            { step: '3', icon: '▶️', text: 'Click "Practice" to drill down into real PYQs for this specific topic' },
          ].map(({ step, icon, text }) => (
            <div key={step} className="flex items-start gap-2.5 bg-white/5 rounded-2xl p-3.5 border border-white/5">
              <span className="w-5 h-5 rounded-full bg-secondary text-white text-[10px] font-black flex items-center justify-center shrink-0 mt-0.5">{step}</span>
              <p className="text-xs text-on-surface-variant leading-relaxed"><span className="mr-1">{icon}</span>{text}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="text-center py-6 text-xs text-on-surface-variant/50">
        <p>Topics sorted by frequency in KSP Police Constable PYQ database · {allQs.length.toLocaleString()} questions analysed</p>
        <p className="mt-1 font-bold text-on-surface-variant">Click any topic → expand trick + formula + diagram + real PYQ with solution</p>
      </div>
    </div>
  );
}
