...
const ExpectedPaper     = lazy(() => import('./components/ExpectedPaper').then(m => ({ default: m.ExpectedPaper })));
...
const PATH_TO_TAB: Record<string, string> = {
  ...
  '/roadmap': 'roadmap', '/planner': 'planner', '/weakareas': 'weakareas', '/expected': 'expected',
  '/admin': 'admin',
};
...
function MainApp() {
  ...
  useEffect(() => {
    localforage.config({ name: 'Karnataka_PC_Mastery_Pro', storeName: 'pyqs_data' });
    localforage.setItem('cached_pyqs', pyqsData).catch(console.error);
  }, []);

  // 🔥 Fire anonymous ping on EVERY app load — captures all visitors before login
  useEffect(() => {
    pingServer({ loginType: 'anonymous', page: window.location.pathname });
  ...
  // On app load: sync both performance + planner data (skip for guests)
  useEffect(() => {
    if (user?.email && user.email !== 'guest@local') {
      syncOnLogin(user.email, { name: user.name, avatar: user.avatar });
      syncPlannerOnLogin();
    }
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleLogin = async (profile: UserProfile) => {
    ...
    // Track every visitor (guest + google) for admin dashboard
    const loginType = profile.email === 'guest@local' ? 'guest' : 'google';
    trackVisit(profile.email, profile.name, loginType);
    // Skip cloud sync for guest users
    if (profile.email !== 'guest@local') {
      syncOnLogin(profile.email, { name: profile.name, avatar: profile.avatar });
  ...
  const handleLogout = () => {
    ...
    auth.signOut().catch(err => console.error('[Auth] Sign out failed:', err));
  };
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
            ...
            <p style={{ color: '#a7f3d0', fontSize: 11, margin: '3px 0 0' }}>
              We've improved the login screen speed & guest access.
            </p>
          </div>
          <button
          ...
          <div className="max-w-screen-xl mx-auto">
            <Suspense fallback={<TabSpinner />}>
              <TabErrorBoundary>
                ...
                {activeTab === 'expected'    && <ExpectedPaper />}
