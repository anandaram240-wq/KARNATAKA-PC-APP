...
async function pushToCloud(
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (meta?.avatar) payload.avatar = meta.avatar;
      // firstSeen: cache in localStorage so we only write it once
      const fsKey = `${emailToPrefix(email)}__firstSeen`;
      if (!localStorage.getItem(fsKey)) {
        payload.firstSeen = serverTimestamp();
        localStorage.setItem(fsKey, new Date().toISOString());
      }
...
export async function syncOnLogin(
  ...
  try {
    ...
    const ok = await pushToCloud(email, merged, meta);
